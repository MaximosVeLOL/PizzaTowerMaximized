if(time > 0) keyboard_key_release(array_length(inputs) - 1);
do {
currentInput = availableInputs[random_range(0, array_length(availableInputs) )];
} until(currentInput != global.settings.keyBinds.p1.jump || time > 100)

//Scoring

var prevScore = selfScore;

selfScore--;
if(time > 300) restart(false);
if( movespeed >= MACHINE_VEL_X_MIN ) selfScore += 5;
if(abs(velocity[1]) > MACHINE_VEL_Y_MIN && movespeed > MACHINE_VEL_X_MIN) movespeed += 2.5;
//if(currentInput == global.settings.keyBinds.p1.jump && movespeed == 0 && time < 10) restart(true);
if(selfScore < MACHINE_SCORE_MIN) {
	//show_message("Failed!");
	restart(true);
}
if(x >= 1984) {
	//global.bestInputs = inputs;
	show_message("Success!");
	restart(false);
}

if(selfScore > prevScore) { //When doing good
	var ourInput = {
		input : currentInput,
		score : selfScore,
	};
	if(array_length(global.bestInputs) < time + 1 || global.bestInputs[time] == 0) {
		global.bestInputs[time] = ourInput;
		return;
	}
	var vsInput = global.bestInputs[time];
	if(vsInput.input == ourInput.input) return;
	if(vsInput.score < ourInput.score) global.bestInputs[time] = ourInput;
	else currentInput = vsInput.input;
}


array_push(inputs, currentInput);
keyboard_key_press(currentInput); //This is kinda stupid...
Log(inputToString(currentInput) + " : " + string(time));
...
//game_set_speed(gamespeed_fps, 5);



time++;
//show_message(global.bestInputs);
